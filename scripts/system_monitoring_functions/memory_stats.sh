get_memory_stats() {
    ...
    print_statements+="$(print_metric "Free RAM (MB)" "$free" $DEFAULT_MEM_LIMIT "under" "Low free memory; consider closing apps")\n"
    print_statements+="$(print_metric "RAM Usage (%)" "$mem_usage_pct" $DEFAULT_MEM_THRESHOLD "over" "High RAM usage")\n\n"
...
get_swap_stats(){
    ...
    print_statements+="$(print_metric "Free Swap (MB)" "$swap_free" $DEFAULT_SWAP_FREE "under" "Low swap space left")\n"
    print_statements+="$(print_metric "Swap Usage (%)" "$swap_usage_pct" $DEFAULT_SWAP_PCT "over" "High swap usage; system may be swapping")\n"
    print_statements+="$(print_metric "Swap In (KB/s)" "$si" $DEFAULT_SWAP_SI_THRESHOLD "over" "System is reading from swap — RAM pressure")\n"
    print_statements+="$(print_metric "Swap Out (KB/s)" "$so" $DEFAULT_SWAP_SO_THRESHOLD "over" "System is writing to swap — RAM may be full")\n"
