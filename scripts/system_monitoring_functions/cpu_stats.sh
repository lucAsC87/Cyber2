...
if [[ -z "$CYBER2_ROOT" ]]; then
    echo "Error: CYBER2_ROOT not set. Please export it before calling this function."
    return 1
fi
...
get_average_cpu_stats() {
    ...
    trap "tput cnorm; stty echo; return" EXIT  # Restore cursor on exit
    ...
    while true; do
        ...
        if (( ${#warnings[@]} > 0 )); then
            ...
            for key in "${!warnings[@]}"; do
                ...
                echo "$(date '+%F %T') - Average - $key: ${warnings[$key]}" >> "$LOG_FILE"
            done
        fi
